% 1. 读取Excel文件
filePath_o = 'G:\cnntest\pythonProject\event\chb01\test1';
filePath_1 = fullfile(filePath_o, '2Channel+EMD');
fileName = 'test_slice_index.xlsx';
filePath = fullfile(filePath_o, fileName);
data = readtable(filePath);
numChannel = 3;
% fileName_1 = ['test_slice_', num2str(numChannel), '_2.csv'];
fileName_1 = ['test_slice_', num2str(numChannel), '_2Channel+EMD.csv'];
% fileName_1 = 'test_slice_1EMD.csv';
% filePath_1 = fullfile(filePath_o, fileName_1);
filePath_1 = fullfile(filePath_1, fileName_1);
data_1 = readtable(filePath_1);
% 获取 FileName 和 Predict 列吧 
fileNames = data.FileName;
% fieldName = ['pred_ratio_actual', num2str(numChannel)];
fieldName = 'pred_ratio_actual';
predictValues = data_1.(fieldName);


% % 设置阈值
% threshold = 0.99;  % 你可以根据需要调整这个值
% % 使用逻辑运算符来设置阈值
% predictValues = predictValues > threshold;
% % 将逻辑值转换为 0 和 1
% predictValues = double(predictValues);
