...
try:
    ...
    for test_folder in test_folders:
        ...
        for channels_numbers in channels_numbers_list:
            ...
            txt_path = os.path.join(f'{test_path}/2Channel+EMD', txt_name)
            # 确保目录存在
            ...
            def process_data(data, index, sample_rate):
                combined_features = compute_EMD(data)
            ...
            csv_name = f'test_slice_{channels_numbers}_2Channel+EMD.csv'
            ...
            checkpoint_callback = ModelCheckpoint(
                filepath=f'{test_path}/2Channel+EMD/best_model_{channels_numbers}.h5',
            ...
            # 定义 CSV 文件路径
            csv_file_path = f'{test_path}/2Channel+EMD/training_history_{channels_numbers}.csv'
            ...
            def plot_training_history(history):
                ...
                plt.tight_layout()
                plt.savefig(f'{test_path}/2Channel+EMD/training_curves_{channels_numbers}.png')
                plt.show()
            ...
            # 测试、计时
            model.load_weights(f'{test_path}/2Channel+EMD/best_model_{channels_numbers}.h5')
            print('start test:')
            ...
            np.save(os.path.join(f'{test_path}/2Channel+EMD', f'test_features_{channels_numbers}.npy'), X_test)

            print('Test accuracy:', test_acc)
            print('Average time:', time_average)
            with open(f"{test_path}/2Channel+EMD/run_time_{channels_numbers}.txt", 'w') as file:
                # 写入文件
                file.write(
                    f"time_total:\n{time_total}\n\ntime_average:\n{time_average}\n\ntest_loss:\n{test_loss}\n\ntest_acc:\n{test_acc}\n\ntest_pre:\n{test_pre}\n\ntest_recall:\n{test_recall}")
            ...
            df = pd.DataFrame(test, columns=['label', f'predict_ratio',
                                             f'pred_ratio_actual'])
            # 保存到CSV文件
            csv_path = os.path.join(f'{test_path}/2Channel+EMD', csv_name)
